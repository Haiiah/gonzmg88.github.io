...
$$
f^\omega(x) = h_2(h_1(x^t diag(\epsilon_1) M_1+b_1)diag(\epsilon_2)M_2+b2)
$$
...
In classification we could set for example $p(y_i \mid f^\omega(x_i))\sim B(\text{sigmoid}(f^\omega(x_i)))$. ($B$ is the bernuilli distribution)
...
Now it is where the magic happens since this stochastic gradient is suspiciously similar to $\text{(SG Eq)}$! We *just* have to choose an error function where $err(f^\omega(x),y) \propto \log(p(y_i\mid f^\omega(x_i)))$ and a regularization $\Omega(\omega)$ that is similar to the divergence!
